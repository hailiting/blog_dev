...
describe("test AutoComplete component", () => {
  ...
  it("test basic AutoComplete behavior", async () => {
    ...
    await wait(() => {
      expect(wrapper.queryByText("ab")).toBeInTheDocument(); // 等ab出现的时候
    });
    // should have two suggestion items
    // 拿到dom节点 wrapper.container
    expect(
      wrapper.container.querySelectorAll(".suggestion-item").length
    ).toEqual(2);
    //click the first item
    fireEvent.click(wrapper.getByText("ab"));
    // testProps.onSelect inSelect会被触发
    expect(testProps.onSelect).toHaveBeenCalledWith({
      value: "ab",
      number: 11,
    });
    expect(wrapper.queryByText("ab")).not.toBeInTheDocument(); // 下拉菜单消失
    //fill the input
