...
/// @title Factory
/// @notice 发布PancakeSwap V3 Pool 池子 和 管理 所属关系，控制  pool protocol fees
contract PancakeV3Factory is IPancakeV3Factory {
    ...
    function createPool(
        ...
        if (info.whitelistRequested) {
            require(
                ...
                "user should be in the white list for this fee tier"
            );
        }
        //
        require(getPool[token0][token1][fee] == address(0));
        pool = IPancakeV3PoolDeployer(poolDeployer).deploy(
            address(this),
            token0,
            token1,
            fee,
    ...
    function collectProtocol(
        ...
        return
            IPancakeV3Pool(pool).collectProtocol(
                recipient,
                amount0Requested,
                amount1Requested
            );
