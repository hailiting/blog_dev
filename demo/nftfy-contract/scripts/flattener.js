...
const SPDX_LICENSE_REGEX = /^\/\/\s*SPDX-License-Identifier:(\s+).*$/gm;
...
async function getProjectRoot() {
  let configFilePath = await findUp("truffle-config.js");
  if (!configFilePath) {
    throw new Error(`
...
async function main(args) {
  ...
  let outputFileIndex = args.indexOf("--outputDir");
  let outputDirPath;

  if (outputFileIndex >= 0) {
    outputDirPath = args[outputFileIndex + 1];

    if (!outputDirPath) {
      console.warn(
        "you havn't provided output file path, ignoring. Usage: truffle-flattener <files> --outputDir <output file path>"
      );
    }
