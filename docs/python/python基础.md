...
- 文件操作
  - open(文件路径, mode="", encoding="")
    ...
    - mode 模式
      ...
      - x: 新建，如果文件存在则报错，如果不存在就新建，然后写入内容，比 w 模式更安全
      - b: 非文本文件 -> bytes，比如 rb, wb, ab, 以 bytes 类型操作数据
      - +: 读写模式：如`r+`, `w+`, `a+`
        - `w+` 在读写之前会清空文件的内容
        - `a+` 永远只能在文件的末尾写入
        - `r+` 读写模式，配合`seek()`和`tell()`方法，可实现更多操作
          - `tell()` 光标定位到哪个位置
          - `seek(offset, from_what)`
            - from_what
              - 如果是 0 表示从文件开头计算
              - 如果是 1，表示从文件读写指针的当前位置开始计算
              - 2 表示从文件的结尾开始计算，默认 0
            - offset 表示偏移量
...
- 定义： `def 函数名字():`
  - 1. 形参，在函数定义的时候，需要准备一些变量来接收信息
    ...
    - 2. 默认值参数，在函数声明的时候给变量一个默认值，如果实参不传递信息，此时默认值生效，默认参数可以指定 key
    - 3. 动态传参
...
- 闭包：本质，内层函数对外层函数的局部变量的使用，此时内层函数被称为闭包函数
  ...
  - 可以避免全局变量被修改
    - 闭包函数必须返回一个函数对象
    - 闭包函数返回的那个函数必须引用外部变量，而返回的那个函数内部不一定要 return 返回值
- 装饰器
