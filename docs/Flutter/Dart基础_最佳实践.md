...
如果需要一个固定长度的结合，使用`new List()`来创建一个固定长度的 list 也是合理的

### 2，不要使用.length 来判断集合是否为空
...
多行代码尽可能使用普通花括号函数体并使用明显的 return 语句。
