...
- 自建后端（从RPC API 从节点读取数据）
  - 订阅（ subscription）
    - newHeads(新的区块), logs, newPendingTransactions
  - 扫块（轮询 Poll / block scanning）
    - 扫块虽然看起来”笨重”，但通常更可靠和更有效:
      - 不会遗漏数据、依赖特定的节点功能
    - 通过 RPC API 从节点读取数据(常见的节点服务商:Alchemy、Infura、QuickNode ...)
      - 区块: `eth_getBlockByHash`、`eth_getBlockByNumber`
      - 交易: `eth_getTransactionByHash`
      - 交易收据: `eth_getTransactionReceipt`
      - 交易日志: `eth_getLogs`(Viem:getLogs )
  - 第三方数据服务商
...
publicClient.getLogs({
  address: '',
  event: parseAbiItem('event Transfer(xx)'),
  args: {
    from: ['0xxx'],
    to: '0xdd',
  },
